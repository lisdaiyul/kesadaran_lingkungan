...
@bot.command()
async def baca(bacaan):
    with open('bacaan.txt', 'r', encoding='utf-8') as t:
        dokumen = t.read()
        await bacaan.send(dokumen)

@bot.command()
async def tulis_ulang(tulisan, *, tuliskan: str):
    with open('bacaan.txt', 'w', encoding='utf-8') as t:
        text = ""
        text += tuliskan
        t.write(text)

@bot.command()
async def tambah(tambahan, *, tambahkan: str):
    with open('bacaan.txt', 'a', encoding='utf-8') as t:
        text = "\n"
        text += tambahkan
        t.write(text)
